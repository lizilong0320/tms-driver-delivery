// In-memory store replacing Prisma for Vercel Serverless
import bcrypt from 'bcryptjs';

// Global in-memory storage
declare global { var __tms_db: any; }
const db = globalThis.__tms_db || (globalThis.__tms_db = initDB());
...
export const prisma = {
  user: {
    findUnique: (args: any) => {
      const { phone, id } = args.where || {};
      if (phone) return db.users.find((u: any) => u.phone === phone) || null;
      if (id) return db.users.find((u: any) => u.id === id) || null;
      return null;
    },
    findFirst: (args: any) => {
      const { phone, id } = args.where || {};
  ...
  driver: {
    ...
    upsert: (args: any) => {
      ...
      db.drivers.push(d); return d;
    },
  },
  shipper: {
    findMany: (args?: any) => db.shippers,
    findUnique: (args: any) => {
      const { id, name } = args.where || {};
      if (id) return db.shippers.find((s: any) => s.id === id) || null;
      if (name) return db.shippers.find((s: any) => s.name === name) || null;
      return null;
    },
    upsert: (args: any) => {
  ...
  warehouse: {
    findMany: (args?: any) => db.warehouses,
    findUnique: (args: any) => {
      const { id, name } = args.where || {};
      if (id) return db.warehouses.find((w: any) => w.id === id) || null;
      if (name) return db.warehouses.find((w: any) => w.name === name) || null;
      return null;
    },
    upsert: (args: any) => {
  ...
  waybill: {
    ...
    updateMany: (args: any) => {
      ...
      db.waybills.forEach((w: any) => { if (filter(w)) Object.assign(w, args.data, { updatedAt: new Date().toISOString() }); });
      return { count: db.waybills.filter(filter).length };
    },
    count: (args?: any) => {
      let list = [...db.waybills];
      const where = args?.where || {};
    ...
    delete: (args: any) => {
      ...
      if (idx >= 0) db.waybills.splice(idx, 1);
    },
    groupBy: (args: any) => {
      const groups: any = {};
      db.waybills.forEach((w: any) => {
        const key = w[args.by[0]] || 'none';
        if (!groups[key]) groups[key] = { _count: { id: 0 } };
        groups[key]._count.id++;
        groups[key][args.by[0]] = key;
      });
      return Object.values(groups).sort((a: any, b: any) => b._count.id - a._count.id).slice(0, args.take || 10);
    },
    upsert: (args: any) => {
      const existing = db.waybills.find((w: any) => w.waybillNo === args.where.waybillNo);
      if (existing) { Object.assign(existing, args.update); return existing; }
      const w = { id: nextId('waybill'), ...args.create, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
      db.waybills.push(w); return w;
    },
  },
  batch: {
    findMany: (args?: any) => {
      ...
      if (where.status !== undefined && where.status !== null) list = list.filter((b: any) => b.status === where.status);
      list.sort((a: any, b: any) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
      if (args?.include) list.forEach((b: any) => resolveIncludes(b, args.include));
      return list;
    },
    findUnique: (args: any) => {
      const { id } = args.where || {};
      let b = db.batches.find((b: any) => b.id === id) || null;
      if (b && args?.include) b = resolveIncludes(b, args.include);
      return b;
    },
    create: (args: any) => {
      const b = { id: nextId('batch'), ...args.data, status: args.data.status || 0, createdAt: new Date().toISOString() };
      db.batches.push(b);
      return args?.include ? resolveIncludes(b, args.include) : b;
    },
    update: (args: any) => {
